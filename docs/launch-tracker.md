...
- **🛡️ Post-deepsec verification (after Phase 1 merges)** — quick browser-side exploit-attempt smoke: (a) open the prod Convex URL via DevTools console + `ConvexHttpClient().query(api.contactSubmissions.list)` → should reject (was wide open). (b) Paste `</script><script>alert(1)</script>` into Karlyne's business name in admin → load `/` → no alert, literal text in `<head>`. (c) On `/admin/contacts`, Export CSV after submitting a form with `firstName: =HYPERLINK(...)` → CSV cell starts with `'`. *(P0 Security § verification)*
...
### 🛡️ P0 Security — Deepsec calibration remediation (added 2026-05-11)

> Calibration run of [Vercel Labs' deepsec](https://github.com/vercel-labs/deepsec) processed 50 of 129 candidate files using Claude Opus 4.7 via the local Claude Code subscription. 27 findings emitted; 26 confirmed actionable after manual code-read verification (1 false positive dropped). Full per-finding write-up: `.deepsec/data/KarlyneMedSpaWebsite/reports/CALIBRATION_FINDINGS.md` (gitignored). Per-bucket remediation plan: `REMEDIATION_PLAN.md` in the same dir.
>
> **Severity:** 1 CRITICAL, 3 HIGH, 17 MEDIUM, 5 BUG. 1 CRITICAL + 3 HIGH are launch-blockers.

- [ ] **[P0 Security § B0] Verify $15.24 calibration cost on Anthropic console** — was it subscription-absorbed (OAuth) or metered? Decision gates running the full pass on the remaining 79 files. *(Decision, not code.)*

- [ ] **[P0 Security § B1] CRITICAL — Convex auth gates on the missing handlers** — `contactSubmissions.{list,countNew,updateStatus}` exposes all PII (CRITICAL). Same root cause spreads to `pabauSync.sync{Reviews,Services,Products,Entity}` (HIGH — public Convex actions write Pabau + DB without auth), `pabauWebhookEvents.{list,stats,findByEventId}` (HIGH — Pabau patient PII leak), `beforeAfter.listAll` (MEDIUM — exposes soft-deleted client photos). Fix is identical: add `await assertAdmin(ctx)` to the listed handlers. Convert `pabauWebhookEvents.{markProcessed,markFailed,markIgnored}` to `internalMutation` and update the webhook route to call them via `ConvexHttpClient.mutation(internal.…)`. Add server-side cooldown to sync actions. Fold in F18 (apply `redactKey` before pushing errors into `summary.errors`) since same file. Files: `convex/contactSubmissions.ts`, `convex/pabauSync.ts`, `convex/pabauWebhookEvents.ts`, `convex/beforeAfter.ts`, `src/app/api/pabau/webhooks/[token]/route.ts`. *Subagent bucket B1 — Phase 1 parallel.*

- [ ] **[P0 Security § B2] MEDIUM — Convex auth helper fail-closed** — `convex/lib/auth.ts:46-54`: when `process.env.ADMIN_EMAILS` is empty/unset, `assertAdmin` silently allows any signed-in Clerk user. Compare with `src/lib/admin/auth.ts:36-41` which throws in production. Make the empty-allowlist path throw unless `CONVEX_ALLOW_ANY_SIGNED_IN=1` is set explicitly. *Subagent bucket B2 — Phase 1 parallel.*

- [ ] **[P0 Security § B3] HIGH — JSON-LD XSS** — `src/app/layout.tsx:148-169, 222-223` injects Convex-stored admin-editable site settings into `<script type="application/ld+json">` via `JSON.stringify` inside `dangerouslySetInnerHTML`. `JSON.stringify` doesn't escape `</`, so an admin-set field containing `</script><script>alert(1)</script>` produces persistent site-wide stored XSS. Verification expanded scope to **`src/app/faq/page.tsx:117`** which has the identical pattern with admin-editable FAQ answers. Fix: new `src/lib/safeJsonLd.ts` helper that escapes `</`, `<!--`, `-->`, U+2028, U+2029; swap into both call sites. *Subagent bucket B3 — Phase 1 parallel.*

- [ ] **[P0 Security § B4] MEDIUM — Blob delete hardening** — `src/app/api/admin/blob/delete/route.ts`: (F6) no in-handler auth; relies on `/api/admin(.*)` middleware. (F7) URL allowlist uses `String.includes('.public.blob.vercel-storage.com')` not a host parse — `https://x.com/?p=.public.blob.vercel-storage.com` passes. (F24) Returns raw `error.message` to client, leaking internals. Fix all three in-file: `auth.protect()` + admin check; replace with `new URL(url).hostname.endsWith(...)`; return generic `"Delete failed"`. *Subagent bucket B4 — Phase 2 parallel.*

- [ ] **[P0 Security § B5] MEDIUM — Blob upload hardening + F12 decision** — `src/app/api/admin/blob/upload/route.ts`: (F12) `allowedContentTypes` includes `image/svg+xml` and max is 100 MB — SVG uploads enable XSS if ever inlined (no inline render today, but defense-in-depth). (F25) HTTP status derived from substring match on `error.message`. Fix: drop SVG from allowlist (pending decision in 💬 Decisions: drop vs sanitize); drop max to 25 MB; define `class ForbiddenError extends Error` and switch on `instanceof` in catch. *Subagent bucket B5 — Phase 2 parallel.*

- [ ] **[P0 Security § B6] MEDIUM — Pabau leads route hardening** — `src/app/api/pabau/leads/route.ts`: (F13) `clientIp()` reads first XFF entry; Vercel appends real IP rather than overwriting, so attacker-supplied XFF wins the rate-limit key. (F14) No CAPTCHA / BotID / per-email dedup before forwarding to paid Pabau API. (F26) `ipHits` Map grows unbounded — never `delete`s empty buckets. Fix: use LAST XFF entry on Vercel; add 24h per-email dedup against `contactSubmissions`; `delete ipHits.get(ip)` when filtered to length 0. F14 has open decision in 💬 Decisions (dedup-only vs add BotID). *Subagent bucket B6 — Phase 2 parallel.*

- [ ] **[P0 Security § B7] MEDIUM — Pabau read-only endpoint hardening** — (F15) `src/app/api/pabau/services/route.ts` uses raw `fetch()` bypassing the `pabauFetch` wrapper that handles throttle, circuit-breaker, and key redaction. (F16) `src/app/api/pabau/lead-sources/route.ts` probes 5–6 Pabau paths per request, no Next-layer caching. (F17) Returns `err.message` + internal `pathTried` to public callers. Fix: swap services to `pabau.services.list()`; add admin auth + 1h `unstable_cache` to lead-sources; scrub the public response. *Subagent bucket B7 — Phase 2 parallel.*

- [ ] **[P0 Security § B8] MEDIUM — Pabau webhook receiver** — `src/app/api/pabau/webhooks/[token]/route.ts` (F20): no body-size cap, no rate limit. Token in URL path is leakage-prone (CDN logs, browser history); once leaked attacker drives unbounded Convex spend. Fix: check `content-length` header before `req.text()` (reject > 100 KB); in-memory per-IP rate limit. **Depends on B1** if B1 converts the webhook mutations to `internalMutation`. *Subagent bucket B8 — Phase 3 parallel.*

- [ ] **[P0 Security § B9] BUG — Pabau client `custom_fields` allowlist** — `src/lib/pabau/client.ts:218-222` spreads attacker-controllable keys into `body`. Today's only caller uses hardcoded keys so it's safe, but the type signature allows any record. Replace with `Object.create(null)` + explicit allowlist (`['treatment_interest', 'website_source']`); tighten the parameter type. *Subagent bucket B9 — Phase 3 parallel.*

- [ ] **[P0 Security § B10] MEDIUM — Admin contacts CSV + mailto + tighter email validation** — `src/app/admin/contacts/page.tsx`: (F10) CSV export only escapes the `message` field, so `firstName: =HYPERLINK(...)` from a visitor's contact-form submission becomes an Excel formula when the admin opens the CSV. (F11) Admin "Reply" mailto href is built from `${sub.email}` which is only validated with `.includes("@")` server-side — `attacker@x.com?cc=victim@example.com&body=phishing` smuggles recipients/body. Fix: install `papaparse` or inline a CSV cell helper that prefixes `=`/`+`/`-`/`@`/`\t`/`\r` with `'`; tighten the email regex in `contactSubmissions.create` to reject URI metacharacters. **Depends on B1** (same Convex file). *Subagent bucket B10 — Phase 3 parallel.*

- [ ] **[P0 Security § B11] MEDIUM — pabauLink scheme validation** — `src/app/membership/page.tsx:254` and `src/app/shop/[slug]/page.tsx:60` render admin-controlled `pabauLink` as `<a href>` with no validation. Convex schemas store it as `v.optional(v.string())` with no `https?:` check; admin forms have `type="url"` but that's browser-only. `target="_blank"` does NOT prevent `javascript:` URLs. Fix: new `src/lib/safeHref.ts` parses with `new URL` (try/catch) and accepts only http/https; apply at both render sites; add server-side validation in `convex/memberships.ts` and `convex/shopProducts.ts` mutations. *Subagent bucket B11 — Phase 2 parallel.*

- [ ] **[P0 Security § B12] BUG — Preview Changes button is broken** — `src/app/admin/customize/page.tsx:489-494` writes `?_preview=…` but `src/components/layout/PreviewOverlay.tsx` reads `_previewSection`/`_previewColors`/`_previewDesignStyle`/`_previewBg` — none match. Clicking "Preview Changes" opens the page with saved defaults, not the unsaved draft. **Decision needed** in 💬 Decisions: (a) align customize page's emitted param keys with what PreviewOverlay reads, vs (b) extend PreviewOverlay to recognize a page-wide `_preview` param (cleaner). *Subagent bucket B12 — Phase 3 parallel.*

- [ ] **[P0 Security § P4] Re-scan + revalidate the remaining 79 files** — only triggered after Phase 1 merges land. `pnpm deepsec scan` (regex pass) → `pnpm deepsec process` (full agent investigation on the remaining files) → `pnpm deepsec revalidate --min-severity HIGH` → `pnpm deepsec export --format md-dir --out ./findings`. Most matcher hits on the remaining files should drop once B1/B2/B3 are merged because the structural patterns will be fixed.
