  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem 1.5rem", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      {submissions === undefined && (
        <p style={{ color: "#374151", textAlign: "center", padding: "3rem 0" }}>Loading submissions...</p>
      )}

      {/* Empty state */}
      {submissions !== undefined && filtered.length === 0 && (
        <div style={{ textAlign: "center", padding: "3rem 0", color: "#374151" }}>
      ...
      {filtered.length > 0 && (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.9375rem" }}>
            <thead>
              <tr style={{ backgroundColor: "#374151", textAlign: "left" }}>
                <th style={{ padding: "0.75rem 1rem", color: "#f9fafb", fontWeight: 600 }}>Name</th>
                <th style={{ padding: "0.75rem 1rem", color: "#f9fafb", fontWeight: 600 }}>Email</th>
                <th style={{ padding: "0.75rem 1rem", color: "#f9fafb", fontWeight: 600 }}>Phone</th>
                <th style={{ padding: "0.75rem 1rem", color: "#f9fafb", fontWeight: 600 }}>Date</th>
                <th style={{ padding: "0.75rem 1rem", color: "#f9fafb", fontWeight: 600 }}>Status</th>
                <th style={{ padding: "0.75rem 1rem", color: "#f9fafb", fontWeight: 600 }}>Message</th>
                <th style={{ padding: "0.75rem 1rem", color: "#f9fafb", fontWeight: 600 }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((sub, idx) => {
                ...
                return (
                  ...
                  >
                    <td style={{ padding: "0.75rem 1rem", color: "#111827", fontWeight: 500, verticalAlign: "top" }}>
                      {sub.firstName} {sub.lastName}
                    </td>
                    <td style={{ padding: "0.75rem 1rem", color: "#111827", verticalAlign: "top" }}>{sub.email}</td>
                    <td style={{ padding: "0.75rem 1rem", color: "#111827", verticalAlign: "top" }}>
                      {sub.phone || "---"}
                    </td>
                    <td style={{ padding: "0.75rem 1rem", color: "#111827", verticalAlign: "top" }}>
                          ...
                          {(["new", "read", "replied"] as ContactStatus[]).map((s) => (
                            <button
                              ...
                              style={{
                                ...
                                padding: "0.4rem 1rem",
                                border: "none",
                                backgroundColor: status === s ? "#f3f4f6" : "transparent",
                    ...
                    <td style={{ padding: "0.75rem 1rem", color: "#111827", verticalAlign: "top", maxWidth: "300px" }}>
                      {isExpanded ? (
                        <div>
                          <p style={{ margin: 0, whiteSpace: "pre-wrap", lineHeight: 1.5 }}>{sub.message}</p>
                          <span style={{ fontSize: "0.8125rem", color: "#9ca3af", marginTop: "0.25rem", display: "inline-block" }}>
